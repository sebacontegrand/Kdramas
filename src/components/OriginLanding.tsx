...
export default function OriginLanding({ onSelect }: OriginLandingProps) {
    ...
    return (
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-6 transition-all duration-1000 ease-in-out ${isVisible ? 'bg-white/40 dark:bg-black/40 backdrop-blur-2xl' : 'bg-transparent backdrop-blur-0 pointer-events-none opacity-0'}`}>
            <div className={`max-w-4xl w-full transition-all duration-700 transform ${isVisible ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-12 opacity-0 scale-95'}`}>
                <div className="text-center mb-12">
                    ...
                    <p className="mt-6 text-zinc-500 dark:text-zinc-400 font-medium text-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {origins.map((origin) => (
                        <button
                            ...
                            className="group relative h-48 rounded-[32px] overflow-hidden transition-all hover:scale-105 active:scale-95 shadow-2xl shadow-zinc-200/50 dark:shadow-none border border-white/80 dark:border-zinc-700/50 bg-white/30 dark:bg-zinc-800/30 backdrop-blur-sm"
                        >
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center p-6">
                                <span className="text-5xl mb-4 transition-transform duration-500 group-hover:scale-125 group-hover:-rotate-6">{origin.icon}</span>
                                <span className="text-lg font-bold text-zinc-800 dark:text-zinc-100 tracking-tight">{origin.name}</span>
                            </div>
